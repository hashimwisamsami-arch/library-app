...
//dummy data for frontend users or who wants the data for data entry
const defaultBooks = [
  {
    id: 1,
    bookCode: "1001",
...
const normalizeBorrowing = (record, fineSettings) => ({
  ...record,
  fineRate:
    Number(record?.fineRate ?? record?.finePerDay ?? fineSettings.amount) || 0,
...
const normalizeStoredBook = (book, fineSettings) => {
  ...
  if (!fallback) {
    return {
      ...
      borrowings: Array.isArray(book.borrowings)
        ? book.borrowings.map((record) =>
            normalizeBorrowing(record, fineSettings),
          )
        : [],
    };
  }
  ...
  return {
    ...
    borrowings: Array.isArray(book.borrowings)
      ...
      : fallback.borrowings.map((record) =>
          normalizeBorrowing(record, fineSettings),
        ),
  };
};
...
const hydrateBooks = (storedBooks, fineSettings) => {
  ...
  const normalizedStoredBooks = storedBooks.map((book) =>
    normalizeStoredBook(book, fineSettings),
  );
  const missingDefaultBooks = defaultBooks
    .filter(
      (defaultBook) =>
        !normalizedStoredBooks.some((book) => book.id === defaultBook.id),
    )
...
const hydrateFineSettings = (storedSettings) => {
  ...
  return {
    amount:
      Number.isFinite(Number(storedSettings.amount)) &&
      Number(storedSettings.amount) >= 0
        ? Number(storedSettings.amount)
        : defaultFineSettings.amount,
    interval: ["day", "week", "month", "year"].includes(storedSettings.interval)
...
const getBookView = (book, fineSettings) => {
  ...
  return {
    ...
    activeLoan,
    status,
    borrowerLabel: activeLoan
      ? `Issued to ${activeLoan.userName}`
      : "Ready for issue",
...
export const LibraryProvider = ({ children }) => {
  ...
  const fetchManualIssues = async () => {
    ...
    try {
      ...
      const issuesUrl = isAdmin
        ? `${API_BOOKS_URL}/issues`
        : `${API_BOOKS_URL}/issues/student`;
  ...
  useEffect(() => {
    ...
    const storedBooks = hydrateBooks(
      readJson(LIBRARY_BOOKS_KEY, defaultBooks),
      storedFineSettings,
    );
    ...
    localStorage.setItem(
      LIBRARY_FINE_SETTINGS_KEY,
      JSON.stringify(storedFineSettings),
    );
  ...
  const returnBook = (bookId, recordId = null) => {
    ...
    setBooks((currentBooks) =>
      currentBooks.map((book) => {
        ...
        const activeLoan =
          book.borrowings.find(
            (record) => record.id === recordId && !record.returnedOn,
          ) ?? getActiveLoan(book);
        ...
        return {
          ...
          borrowings: book.borrowings.map((record) =>
            record.id === activeLoan.id
              ? { ...record, returnedOn: todayIso }
              : record,
          ),
        };
      }),
        ...
        return {
          ...
          borrowings: book.borrowings.map((record) =>
            record.id === targetRecordId
              ? {
                  ...record,
                  manualFine: nextAmount,
                  fineCleared: nextAmount > 0 ? false : record.fineCleared,
                }
              : record,
          ),
        };
  ...
  const issueManualBooksToStudent = async ({
    userEmail,
    studentDetails,
    books: manualBooks,
  }) => {
    try {
      ...
      return {
        ok: false,
        error: data.message || "Failed to issue manual books.",
      };
    } catch (error) {
  ...
  const returnIssuedRecord = async ({ source, recordId, bookId = null }) => {
    ...
    try {
      const response = await fetch(
        `${API_BOOKS_URL}/issues/${recordId}/return`,
        {
          method: "PUT",
          headers: getHeaders(),
        },
  ...
  const clearFineForRecord = async ({ source, recordId, bookId = null }) => {
    ...
    try {
      const response = await fetch(
        `${API_BOOKS_URL}/issues/${recordId}/clear-fine`,
        {
          method: "PUT",
          headers: getHeaders(),
        },
  ...
  const allRecords = manualIssues.map((issue) =>
  ...
  const studentSummaries = studentAccounts.map((account) => {
    ...
    const overdueRecords = activeRecords.filter(
      (record) => record.liveStatus === "Overdue",
    );
    ...
    return {
      ...
      fineClearedCount: activeRecords.filter((record) => record.fineCleared)
        .length,
